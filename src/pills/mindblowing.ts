...
function MINDBLOWING(player: EntityPlayer) {
    ...
    let forgot;
    let distance = 20;
    forgot = Isaac.Spawn(3, 900, 0, Vector(320, 280), Vector(0, 0), undefined);
    print(forgot.Size);
    forgot.Size = 10;
    let originalLink = Isaac.Spawn(
        1000,
        114,
        0,
        forgot.Position.add(Vector(0, distance)),
        Vector(0, 0),
        undefined,
    );
    //originalLink.ToEffect().SetRadii(5.0, 10.0)
    (originalLink.ToEffect() as EntityEffect).Scale = 60; //120 is the usual forgotten
